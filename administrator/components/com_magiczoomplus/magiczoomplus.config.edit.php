    ...
    ?>
        ...
        <table class="adminlist magictoolbox" style="width: 100%;">
            ...
            <?php
                ...
                if($param['id'] == 'enable-effect' && $configs[1]['name'] != 'details') {
                    foreach($param['values'] as $vId => $vVal) {
                        if($vVal == 'Swap images only') {
                            unset($param['values'][$vId]);
                            break;
                        }
                    }
                }
            ...
            ?>
                <tr class="row<?php echo $i%2?>">
                    <th>
                        ...
                        ?>
                        <b class="search-target"><?php echo $labels['1'];?></b><?php if(isset($param['description']) && !empty($param['description'])) { ?> (<a class="magictooltip" title="<?php echo htmlspecialchars($param['description'], ENT_COMPAT, 'UTF-8'); ?>" >?</a>)<?php } ?><span>(<?php echo $labels['2'];?>)</span>
                    </th>
                    <?php foreach($configs as $k => $c): if($k == 0) continue;?>
                        <td>
                            <?php
                                //NOTE: display icon for 'magicscroll' option
                                if($param['id'] == 'magicscroll') {
                                    echo "<img id=\"magicscroll_icon\" src=\"".JURI::base()."components/com_magiczoomplus/image/magicscroll.png"."\" />";
                                }
                                switch($param['type']){
                                    case 'text':
                                    case 'num':
                                ...
                                if($param['id'] == 'magicscroll') {
                                    echo '<div class="description">'.$param['description'].'</div>';
                                }
                            ?>
                        </td>
                    <?php endforeach ?>
